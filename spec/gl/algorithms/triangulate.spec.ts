...
describe("triangulate", () => {
    ...
    it("adjusting subsets", async () => {
        const r = await fetch("obj/mh/cube.obj")
        const cube0 = new WavefrontObj("cube.obj", await r.text())
        cube0.materialSubset = new Map([
            ["a", { start: 4, length: 8 }]
        ])
        cube0.groupSubset = new Map([
            ["b", { start: 8, length: 4 }]
        ])
        const cube1 = triangulate(cube0)
        expect(cube1.materialSubset).to.not.be.undefined
        expect(cube1.materialSubset?.get("a")).to.deep.equal({ start: 6, length: 12 })

        expect(cube1.groupSubset).to.not.be.undefined
        expect(cube1.groupSubset?.get("b")).to.deep.equal({ start: 12, length: 6 })
    })
})