...
describe("nearestPointBetweenLines", () => {
    ...
    it("should handle degenerate case where directionA is zero", () => {
        // originA is a point, line B passes through (1,1,1) in x-direction
        const originA = vec3.fromValues(0, 0, 0)
        const directionA = vec3.fromValues(0, 0, 0)
        const originB = vec3.fromValues(1, 1, 1)
        const directionB = vec3.fromValues(1, 0, 0)

        const result = nearestPointBetweenLines(originA, directionA, originB, directionB)

        // Closest point on line B to originA is (0,1,1)
        expect(result.a).toBeCloseTo(0, 1e-6)
        expect(result.b).toBeCloseTo(-1, 1e-6)
    ...
    it("should handle degenerate case where directionB is zero", () => {
        ...
        const directionA = vec3.fromValues(1, 0, 0)
        const originB = vec3.fromValues(1, 1, 1)
        const directionB = vec3.fromValues(0, 0, 0)

        const result = nearestPointBetweenLines(originA, directionA, originB, directionB)

        // Closest point on line A to originB is (1,0,0)
